...
<c:choose>
    <c:when test="${not empty transactionsFound}">
        <c:choose>
            <c:when test="${transactionsFound eq 'RESULT_FOUND'}">
                <h3>
                ...
                <table id="clientRecord"
                    ...
                    <c:forEach items="${transactionsResults}" var="reservations">
                        <form action="cancelreservationForm" method=post onsubmit="return confirm('Please confirm cancellation of  ${reservations.licensePlate}?');">
                            ...
                            <tr>
                                <td>${reservations.licenseNumber}</td>
                                <td>${reservations.licensePlate}</td>
                                <td>${reservations.rentedFrom}</td>
